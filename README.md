...
**Note on spec files:** If [more than one user ID or group ID is mapped](https://github.com/opencontainers/runtime-spec/blob/master/config-linux.md) into a container by altering the spec file generated by `runc spec --rootless`, the /etc/subuid and /etc/subgid files must be setup to allow for UID/GID mapping to succeed (see newuidmap(1) and subuid(5) man pages).
